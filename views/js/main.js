...
var pizzaElementGenerator = function (i) {
    ...
    // Added the following line so we don't have to change inline styles
    // when resizing the pizzas.  We would now just change the classList.
    pizzaContainer.classList.add("medium");

    // The following line would no longer be used.
...
function updatePositions() {
    ...
    // Reset to false so we can come back to this function
    // on the next scroll event.
    updateNeeded = false;

    var items = document.getElementsByClassName("mover");

    // This variable is used to build the html that will be
    // inserted in the DOM after the loop is over.
    var html = '';

    // I need to know the number of columns and s within
    // this function so I go ahead and get them now.
    var cols = getDims().cols;
    var s = getDims().s;
    var phase = 0;
    for (var i = 0; i < items.length; i++) {
        // I calculate the phase just as before but instead of % 5, I do
        // % (cols + 1).  This makes it impossible for two rows of pizzas
        // having the same phase.
        phase = Math.sin((latestScrollY / 1250)+(i % (cols + 1)));

        // Here I build the html by calling moverTemplate with the top and left
        // value of the current element.
        html += moverTemplate((Math.floor(i / cols) * s), (i % cols) * s + 100 * phase);
    }
...
// This function generates the mover elements upon page load and upon any resize events.
// This is a new function that replaces the functionality that was previously in the
...
function generateMovers() {
    ...
    // Instead of 200 elements, we now generate (cols * rows) elements
    for (var i = 0; i < cols * rows; i++) {
        // Here we get the html for the current element by calling moverTemplate.
        html += moverTemplate((Math.floor(i / cols) * s), (i % cols) * s);
    }
...
// This function take in a top and left value and returns an img element with class mover
// and proper position values.
function moverTemplate(top, left) {
    var template = '<img class="mover" src="images/pizza.png" style="height: 100px; width: 73.333px; ';
    template += 'top: ' + top + 'px; ';
    template += 'left: ' + left + 'px;">';

    return template;
}

// Returns the optimum number of columns and rows for the .mover pizza elements
// base on the size of the window.
function getDims() {
...
// Here we do the rAF optimization as explained the the article (see README.md).

// Global var to store scrollY info.
var latestScrollY = 0;

// Flag to indicate that we need to updatePositions.
var updateNeeded = false;

// Modified the scroll event callback to only update latestScrollY and
// update the positions only when necessary.
window.addEventListener('scroll', function () {
    latestScrollY = window.scrollY; // Update scrollY info on every scroll event.
    requestUpdate();
});

// This function ensures that the positions are only updated if there has been
// a scroll and only once per animation frame.
...
// Regenerate the sliding pizzas when the window resize.
